...
def mobility_coverage(scenario_num):
    """
    Implements mobility-based coverage control for the given scenario number.
    """
    ...
    def calculate_all_costs(x_si, robot_assignments, D_squared, Vr,phi_q):
        """
        ...
        for i in range(N):
            ...
            if np.any(valid_points):
                ...
                temporal_cost += np.sum(phi_q[valid_points] * (dist_sq / (Vr[i] ** 2))) * (res ** 2)
    ...
    def plot_robot_trajectories(robot_trajectories, initial_positions, final_positions, sensor_types, scenario_num):
        ...
        ax.set_title(f"Robot Trajectories for Mobility Coverage: Scenario {scenario_num}", fontsize=14)
        ax.grid()
        ...
        # Save plot
        plt.savefig(f"data/mobility_trajectories_case_{scenario_num}.png")
        plt.close()
    ...
    for k in range(iterations):
        ...
        centroids, weights, costs = calculate_all_costs(x_si, robot_assignments, D_squared, Vr,phiq)
    ...
    pd_cost.to_csv(f"data/mobility_coverage_cost_{scenario_num}.csv", index=False)

    with open(f"data/mobility_coverage_case_{scenario_num}.txt", "w") as f:
...
# Run mobility coverage for all 10 scenarios
for scenario in range(1, 11):
    print(f"Running mobility coverage for scenario {scenario}")
    mobility_coverage(scenario)
