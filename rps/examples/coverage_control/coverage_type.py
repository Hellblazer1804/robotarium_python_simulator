...
def type_coverage(scenario_num):
    """
    Implements type-based coverage control for the given scenario number.
    """
    ...
    # Importance values for each robot (all equal for this implementation)
    importance = {}
    for sensor_type, robots in sensor_types.items():
        importance[sensor_type] = {robot: 1.0 for robot in robots}
    ...
    def plot_robot_trajectories(robot_trajectories, initial_positions, final_positions, sensor_types, scenario_num):
        ...
        ax.set_title(f"Robot Trajectories for Type Coverage: Scenario {scenario_num}", fontsize=14)
        ax.grid()
        ...
        # Save plot
        plt.savefig(f"data/type_trajectories_case_{scenario_num}.png")
        plt.close()
    ...
    for k in range(iterations):
        ...
        # Calculate distances for all grid points to all robots
        diff = grid_points[:, :, None] - x_si[:, None, :]  # (2, M, N)
        D_squared = np.sum(diff ** 2, axis=0)  # (M, N)

        # Initialize robot assignments
        robot_assignments = np.full(grid_points.shape[1], -1)

        # Process each point in the grid
        for ix in range(grid_points.shape[1]):
            for sensor_type, robots in sensor_types.items():
                distances = np.full(N, np.inf)

                # Calculate distances for robots with this sensor type
                for robot in robots:
                    distances[robot] = D_squared[ix, robot]

                # Find the closest robot within the current sensor type
                min_index = robots[np.argmin([distances[r] for r in robots])]

                # If this is the closest robot of any sensor type so far, assign the point to it
                if robot_assignments[ix] == -1 or distances[min_index] < D_squared[ix, robot_assignments[ix]]:
                    robot_assignments[ix] = min_index
    ...
    with open(f"data/type_coverage_case_{scenario_num}.txt", "w") as f:
...
# Run type coverage for all 10 scenarios
for scenario in range(1, 11):
    print(f"Running type coverage for scenario {scenario}")
    type_coverage(scenario)
